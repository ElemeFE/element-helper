  ...
  setView(url) {
    this.stylePromise.then(() => {
      ...
      const notice = ({
        ...
        'en-US': `Version: ${this.version_}，Looking online examples in <a href="${href}">browser</a>`
      })[this.language_];
      this.element.innerHTML = `<style type="text/css">${DocView.DOC_STYLE}</style><div class="move-mask"></div><div class="el-loading-mask"><div class="el-loading-spinner"><svg viewBox="25 25 50 50" class="circular"><circle cx="50" cy="50" r="20" fill="none" class="path"></circle></svg></div></div><div class="docs-notice">${notice}</div><iframe id="doc-frame" src="${iframeSrc}" frameborder="0" marginheight="0" marginwidth="0" frameborder="0" width="100%" height="100%" scrolling="auto"></iframe>`;
